...
import { ArrowUpCircle, ArrowLeft, Search, Eye } from 'lucide-react';
...
/**
 * Stock Out History page displays all past stock out (Sale) operations.
  ...
  const fetchOperations = async () => {
    ...
    try {
      const response = await apiClient.get(
        `${API_ENDPOINTS.STOCK.OPERATIONS}?type=stock_out&page=${pagination.page}&limit=${pagination.limit}`
      );
      ...
      console.error('Failed to fetch stock out operations:', error);
    } finally {
    ...
    >
            ...
            <div className="flex flex-col gap-4 py-4 md:gap-6 md:py-6 px-4 lg:px-6">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <Button
                      variant="ghost"
                      size="icon"
                      onClick={() => router.push(APP_PATHS.STOCK_OUT)}
                      className="h-8 w-8"
                    >
                  ...
                  <p className="text-muted-foreground mt-1">
                    View all past stock out (Sale) operations
                  </p>
                </div>
                <Button onClick={() => router.push(APP_PATHS.STOCK_OUT)}>
              ...
              <Card>
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div>
                      <CardTitle>Stock Out Operations</CardTitle>
                      <CardDescription>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="relative">
                        ...
                        <Input
                          placeholder="Search by order, customer, warehouse, SKU..."
                          value={searchTerm}
                ...
                <CardContent>
                  {isLoadingOperations ? (
                    <TableSkeleton
                      rows={10}
                      columns={9}
                      columnWidths={[120, 150, 120, 150, 150, 100, 100, 180, 80]}
                    />
                  ) : filteredOperations.length === 0 ? (
                    <div className="text-center py-8 text-muted-foreground">
                      <ArrowUpCircle className="size-12 mx-auto mb-4 opacity-50" />
                      <p>No stock out operations found</p>
                      <p className="text-sm mt-2">
                        {searchTerm ? 'Try a different search term' : 'Create your first stock out operation'}
                      </p>
                    </div>
                  ) : (
                            ...
                            return (
                              <TableRow key={operation.id}>
                                <TableCell className="font-medium">
                                  {formatDate(operation.saleDate || operation.createdAt)}
                                </TableCell>
                                <TableCell>
                                  {operation.warehouse ? (
                                    <div>
      ...
      <Dialog open={viewDialogOpen} onOpenChange={setViewDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Stock Out Operation Details</DialogTitle>
            <DialogDescription>
              Complete details of the stock out operation
            </DialogDescription>
          </DialogHeader>
          {selectedOperation && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm">{formatDate(selectedOperation.saleDate || selectedOperation.createdAt)}</p>
                </div>
                <div>
