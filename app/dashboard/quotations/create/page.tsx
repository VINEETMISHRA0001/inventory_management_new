...
export default function CreateQuotationPage() {
  ...
  const [items, setItems] = useState<QuotationItem[]>([
    { sku: '', productName: '', quantity: 1, unitPrice: 0, total: 0 },
  ]);
  ...
  const addItem = () => {
    setItems([...items, { sku: '', productName: '', quantity: 1, unitPrice: 0, total: 0 }]);
  };
  ...
  const handleSkuSelect = (index: number, sku: string, product?: any) => {
    ...
    newItems[index] = {
      ...
      total: newItems[index].quantity * (product?.price || 0),
    };
    setItems(newItems);
  };
  ...
  const calculateTotals = () => {
    ...
    const tax = (subtotal - discount) * ((formData.taxRate || 0) / 100);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const validItems = items.filter(item => item.sku && item.quantity > 0 && item.unitPrice > 0);
    ...
    try {
      ...
      const response = await apiClient.post(API_ENDPOINTS.QUOTATIONS.BASE, payload);
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Enter customer details for this quotation</CardDescription>
                  </CardHeader>
                    ...
                    {items.map((item, index) => (
                      <div key={index} className="grid grid-cols-12 gap-4 p-4 border rounded-lg">
                        <div className="col-span-4 space-y-2">
                          ...
                          <SkuAutocomplete
                            value={item.sku}
                            onChange={(sku, product) => handleSkuSelect(index, sku, product)}
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-3">
                      ...
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium text-muted-foreground">-₹{discount.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                      ...
                      <div className="border-t pt-3 mt-3">
                        <div className="flex justify-between">
                          ...
                          <span className="text-lg font-bold">₹{total.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
